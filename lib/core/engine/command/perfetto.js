...
export class PerfettoTrace {
  ...
  async uploadConfigFile(android, config) {
    ...
    return android._uploadFile(destinationFilename, "/sdcard/browsertime-perfetto-config.txt");
  }
  ...
  async start(config=defaultConfig) {
    ...
    const tracedEnabled = await this.android._runCommandAndGet(`getprop persist.traced.enable`)
    if (tracedEnabled !== "1") {
    ...
    const output = await this.android._runCommandAndGet('cat /sdcard/browsertime-perfetto-config.txt | perfetto -c - --txt --detach=browsertime -o /data/misc/perfetto-traces/trace');
    log.info(`perfetto output: ${output}`);
  ...
  async stop() {
    ...
    log.info('Starting perfetto tracing.');
    const output = await this.android._runCommandAndGet('perfetto --attach=browsertime --stop');
    log.info(`perfetto output: ${output}`);
    ...
    if (!stopSuccessful) {
      throw new Error('Perfetto process faied or ended suddently.');
    }
